...
class InteractiveKeyWordRepository(BaseRepository[InteractiveKeyWordModel]):
    ...
    async def get_keywords_by_paragraph_id(
        self, paragraph_id: UUID
    ) -> Sequence[InteractiveKeyWordModel]:
        try:
            stmt = (
...
class KeyWordTypeRepository(BaseRepository[KeyWordTypeModel]):
    ...
    async def get_types_with_video_styles(
        self, video_id: UUID
    ) -> Sequence[KeyWordTypeModel]:
        try:
            stmt = (
...
class VideoKeywordTypeStyleRepository(BaseRepository[VideoKeywordTypeStyleModel]):
    ...
    async def get_styles_by_video_id(
        self, video_id: UUID
    ) -> Sequence[VideoKeywordTypeStyleModel]:
        try:
            stmt = (
    ...
    async def get_style_by_video_and_type(
        self, video_id: UUID, keyword_type_id: UUID
    ) -> VideoKeywordTypeStyleModel | None:
        try:
            stmt = select(VideoKeywordTypeStyleModel).where(
